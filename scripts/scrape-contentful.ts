...
async function main() {
  ...
  if (!spaceId || !accessToken) {
    console.error(
      "Missing CONTENTFUL_SPACE_ID or CONTENTFUL_ACCESS_TOKEN in .env / .env.local",
    );
    process.exit(1);
  }
  ...
  if (author?.fields) {
    ...
    const description = (
      author.fields as {
        description?: { nodeType: string; content?: unknown[] };
      }
    ).description;
    let body = "";
    ...
    writeMdx(
      join(CONTENT_DIR, "resume.mdx"),
      { title },
      body || "Add your summary here.",
    );
  ...
  for (const item of projectsRes.items) {
    ...
    let body = "";
    if (
      f.projectDescription?.nodeType === "document" &&
      f.projectDescription.content
    ) {
      try {
        const result = documentToMarkdown({
          data: {},
        ...
        console.warn(
          `  Could not convert project "${name}" description to markdown:`,
          e,
        );
      }
    }
