...
function isVideoUrl(url: string): boolean {
  return VIDEO_EXTENSIONS.test(url) || url.includes('youtube.com') || url.includes('youtu.be') || url.includes('vimeo.com')
}

function buildMediaItem(url: string, alt: string): ProjectMediaItem {
  const normalized = url.startsWith('http') ? url : url.startsWith('/') ? `https://arthurdias.dev${url}` : `https://arthurdias.dev/${url}`
...
export function ProjectMediaCarousel({ urls, projectName, fallbackImage }: ProjectMediaCarouselProps) {
  ...
  return (
    <div className="relative mx-auto w-full max-w-3xl overflow-hidden rounded-2xl">
      ...
      >
        ...
        {items.length > 1 ? (
          <>
            <CarouselPrevious className={`left-2 md:left-4 ${solidButtonClass}`} />
            <CarouselNext className={`right-2 md:right-4 ${solidButtonClass}`} />
          </>
        ) : null}
      </Carousel>
    </div>
